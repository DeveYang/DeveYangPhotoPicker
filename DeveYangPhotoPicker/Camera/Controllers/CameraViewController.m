...
@interface CameraViewController ()<UIActionSheetDelegate,UICollectionViewDataSource,UICollectionViewDelegate,AVCaptureMetadataOutputObjectsDelegate,CameraImageViewDelegate,CameraViewDelegate>//,ZLPhotoPickerBrowserViewControllerDataSource,ZLPhotoPickerBrowserViewControllerDelegate>
@property (weak,nonatomic) CameraView *caramView;
...
- (void) initialize
{
    ...
    CameraView *caramView = [[CameraView alloc] initWithFrame:CGRectMake(0, 40, self.view.width, self.view.height - 40 - BOTTOM_HEIGHT)];
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    Camera *camera = self.images[indexPath.item];
    
    CameraImageView *lastView = [cell.contentView.subviews lastObject];
    if(![lastView isKindOfClass:[CameraImageView class]]){
        // 解决重用问题
        UIImage *image = camera.thumbImage;
        CameraImageView *imageView = [[CameraImageView alloc] init];
...
- (void)deleteImageView:(CameraImageView *)imageView{
    ...
    for (Camera *camera in self.images) {
        UIImage *image = camera.thumbImage;
